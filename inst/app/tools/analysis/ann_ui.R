...
ann_pred_args <- as.list(if (exists("predict.ann")) {
  formals(predict.ann)
} else {
...
ann_pred_plot_args <- as.list(if (exists("plot.model.predict")) {
...
output$ui_ann_rvar <- renderUI({
  ...
  selectInput(
    inputId = "ann_rvar", label = "Response variable:", choices = vars,
...
output$ui_ann_lev <- renderUI({
  ...
  selectInput(
    inputId = "ann_lev", label = "Choose level:",
    choices = levs,
...
output$ui_ann_evar <- renderUI({
  ...
  selectInput(
    inputId = "ann_evar", label = "Explanatory variables:", choices = vars,
...
output$ui_ann_wts <- renderUI({
  ...
  selectInput(
    inputId = "ann_wts", label = "Weights:", choices = vars,
...
output$ui_ann <- renderUI({
  tagList(
    ...
    conditionalPanel(
      ...
      wellPanel(
        selectInput(
          "ann_predict", label = "Prediction input:", reg_predict,
          selected = state_single("ann_predict", reg_predict, "none")
        ),
        conditionalPanel(
          "input.ann_predict == 'data' | input.ann_predict == 'datacmd'",
          selectizeInput(
            inputId = "ann_pred_data", label = "Predict for profiles:",
            choices = c("None" = "", r_data$datasetlist),
            selected = state_single("ann_pred_data", c("None" = "", r_data$datasetlist)), multiple = FALSE
          )
        ),
        conditionalPanel(
          "input.ann_predict == 'cmd' | input.ann_predict == 'datacmd'",
          returnTextAreaInput(
            "ann_pred_cmd", "Prediction command:",
    ...
    conditionalPanel(
      condition = "input.tabs_ann == 'Plot'",
      wellPanel(
        selectInput(
          "ann_plots", "Plots:", choices = ann_plots,
    ...
    wellPanel(
      radioButtons(
        "ann_type", label = NULL, c("classification", "regression"),
      ...
      tags$table(
        tags$td(numericInput(
          "ann_size", label = "Size:", min = 1, max = 20,
          value = state_init("ann_size", 1), width = "77px"
        )),
        tags$td(numericInput(
          "ann_decay", label = "Decay:", min = 0, max = 1,
      ...
      conditionalPanel(
        condition = "input.tabs_ann == 'Summary'",
        tags$table(
    ...
    help_and_report(
      modal_title = "Neural Network (ANN)",
      fun_name = "ann",
...
output$ann <- renderUI({
  ...
  ann_output_panels <- tabsetPanel(
    ...
    tabPanel(
      "Plot", plot_downloader("ann", height = ann_plot_height),
...
observeEvent(input$ann_report, {
  ...
  update_report(
    inp_main = clean_args(ann_inputs(), ann_args),
    fun_name = "ann",
    inp_out = inp_out,
    outputs = outputs,
    wrap = TRUE,
